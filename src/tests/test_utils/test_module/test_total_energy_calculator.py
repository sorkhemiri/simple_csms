...
class TotalEnergyCalculatorTestCase:
    ...
    async def test_testcase_result():
        result = await total_energy_calculator(meter_start=Decimal(1204307), meter_stop=Decimal(1215230))
    ...
    async def test_start_meter_gt_stop_meter():
        with pytest.raises(HTTPException):
            await total_energy_calculator(meter_start=Decimal(1215230), meter_stop=Decimal(1204307))
